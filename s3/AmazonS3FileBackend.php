...
class AmazonS3FileBackend extends FileBackendStore {
	...
	protected function doCreateInternal( array $params ) {
		...
		$ret = $this->runWithExceptionHandling( __METHOD__, function ()
	...
	protected function doDeleteInternal( array $params ) {
		...
		$ret = $this->runWithExceptionHandling( __METHOD__, function () use ( $bucket, $key ) {
	...
	/**
	 * @param string $caller For logging: __METHOD__ that called this runWithExceptionHandling().
	...
	private function logException( S3Exception $e, $caller ) {
		...
		$this->logger->error(
			'S3FileBackend: exception {exception} from {commandName} ({commandParams}) ' .
			'in {func}: {errorMessage}',
			[
				'exception' => $e->getAwsErrorCode(),
				'commandName' => $command->getName(),
				'commandParams' => FormatJson::encode( $command->toArray(), true ),
				'func' => $caller,
